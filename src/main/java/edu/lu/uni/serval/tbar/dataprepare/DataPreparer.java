...
//		libPath.add(IOUtils.class.getProtectionDomain().getCodeSource().getLocation().getFile());
    ...
    */
		// selab: modified code
    //srcPath = buggyProjectParentPath + buggyProject + "/src/";
	srcPath = buggyProjectParentPath + buggyProject + "/";
    String buggyProjectDir = this.buggyProjectParentPath + buggyProject;
    this.validPaths = new File(buggyProjectDir).exists();
    if (!this.validPaths) {
      System.out.println("Invalid Path: " + buggyProjectDir);
      return;
    }
		// selab: end
    }
